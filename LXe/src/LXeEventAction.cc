...
LXeEventAction::LXeEventAction(LXeRecorderBase* r)
  : fRecorder(r),fSaveThreshold(0),fPlasticCollID(-1),fPMTCollID(-1),fScintCollID(-1),fVerbose(0),
...
void LXeEventAction::BeginOfEventAction(const G4Event* anEvent){
  ...
  if(fPlasticCollID<0)
    fPlasticCollID=SDman->GetCollectionID("plasticCollection");
  if(fPMTCollID<0)
    fPMTCollID=SDman->GetCollectionID("pmtHitCollection");

  if(fScintCollID<0)
   fScintCollID=SDman->GetCollectionID("scintHitCollection");

  /*if(fPLASTICCollID<0)
    fPLASTICCollID=SDman->GetCollectionID("plasticHitCollection");*/
...
void LXeEventAction::EndOfEventAction(const G4Event* anEvent){
 ...
 LXeUserEventInformation* eventInformation
  ...
  LXePlasticHitsCollection* plasticHC = 0;
 ...
 // LXePLASTICHitsCollection* plasticHC = 0;
  ...
  if(hitsCE){
    if(fPlasticCollID>=0)plasticHC = (LXePlasticHitsCollection*)(hitsCE->GetHC(fPlasticCollID));
...
//Hits in water tank
  if(scintHC){
    ...
    for(int i=0;i<n_hit;i++){ //gather info on hits in scintillator
     auto hit2 = static_cast<LXeScintHit*>(scintHC->GetHit(i));
     G4ParticleDefinition* partId2 = hit2->GetPartId();
  ...
  //Fill histogram
      analysisManager->FillH1(4, particleName);
      analysisManager->FillH1(5, hit2->GetTimeTank());
...
//Hits in scintillator
  if(plasticHC){
    int n_hit2 = plasticHC->entries();
    G4ThreeVector  eWeightPosPL(0.);
    G4double edepPL;
    G4double edepMaxPL=0;
    G4int particleNamePL;

    for(int i=0;i<n_hit2;i++){ //gather info on hits in scintillator
     auto hit1 = static_cast<LXePLASTICHit*>(plasticHC->GetHit(i));
     G4ParticleDefinition* partIdPL = hit1->GetPartIdPL();
     //Given a number for each particle
  if ( partIdPL == Gamma)
            {
              particleNamePL = 1;//gamma
           // analysisManager->FillH1(2,gamma);
            }
          else if (  partIdPL ==Posit )
            {                       //e+
              particleNamePL = 2;
             // analysisManager->FillH1(3,Posit1);
            }
          else if (  partIdPL == Elect)
            {                       //e-
              particleNamePL = 3 ;
              //analysisManager->FillH1(4,Elect1);
            }
          else if (  partIdPL == Muonp)
            {                       //mu+
              particleNamePL = 5;
              //analysisManager->FillH1(5,Muonp1);
            }
          else if (  partIdPL == Muonm)
            {                       //mu-
              particleNamePL = 6;
             // analysisManager->FillH1(6,Muonm1);
            }
     //G4String PartId = particleTable->GetParticleName(parId);
  //Fill histogram
      analysisManager->FillH1(8, particleNamePL);

      edepPL=(*plasticHC)[i]->GetEdepPL();
      eventInformation->IncEDep(edepPL); //sum up the edep
      eWeightPosPL += (*plasticHC)[i]->GetPosPL()*edepPL;//calculate energy weighted pos
      if(edepPL>edepMaxPL){
        edepMaxPL=edepPL;//store max energy deposit
        G4ThreeVector posMaxPL=(*plasticHC)[i]->GetPosPL();
        eventInformation->SetPosMax(posMaxPL,edepPL);
      }
    }
    if(eventInformation->GetEDep()==0.){
      if(fVerbose>0)G4cout<<"No hits in the scintillator this event."<<G4endl;
      ...
      //Finish calculation of energy weighted position
      eWeightPosPL/=eventInformation->GetEDep();
      eventInformation->SetEWeightPos(eWeightPosPL);
      if(fVerbose>0){
        G4cout << "\tEnergy weighted position of hits in LXe : "
               << eWeightPosPL/mm << G4endl;
      }
    }
    if(fVerbose>0){
    G4cout << "\tTotal energy deposition in scintillator : "
...
/* if(plasticHC){
   // G4ThreeVector reconPos(0.,0.,0.);
    G4int plastics=plasticHC->GetSize();//entries
   // G4double hitime;
    
    //Gather info from all PMTs
    for(G4int i=0;i<plastics;i++){
    // hitime =(*pmtHC)[i]->GetTime();
     auto hit2 = static_cast<LXePLASTICHit*>(plasticHC->GetHit(i));
      eventInformation->IncHitCount((*plasticHC)[i]->GetPhotonCountPL());
     // reconPos+=(*pmtHC)[i]->GetPMTPos()*(*pmtHC)[i]->GetPhotonCount();
      // fill ntuple
      //analysisManager->FillNtupleDColumn(0,hit->GetTime());
       analysisManager->FillH1(4, hit2->GetTimePL());
